...
class GeminiPricingStore(context: Context) {
    ...
    private fun defaultPricing(model: String): GeminiPricing = when (model) {
        AiModelConfig.GEMINI_PRIMARY -> GeminiPricing(
            model = model,
        ...
        AiModelConfig.GEMINI_FALLBACK -> GeminiPricing(
            model = model,
            inputUsdPerMillion = BigDecimal("0.25"),
            outputUsdPerMillion = BigDecimal("1.50"),
            cachedInputUsdPerMillion = BigDecimal("0.025"),
            source = SOURCE_BUNDLED,
            effectiveDate = "2026-09-15",
        )
        else -> GeminiPricing(
