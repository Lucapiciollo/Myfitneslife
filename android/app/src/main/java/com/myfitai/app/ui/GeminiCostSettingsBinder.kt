...
object GeminiCostSettingsBinder {
    ...
    private fun showPricingEditor(activity: SettingsActivity, pricingStore: GeminiPricingStore, pricing: GeminiPricing, refresh: () -> Unit) {
        ...
        val dialog = MaterialAlertDialogBuilder(activity).setTitle("Prezzi Gemini manuali").setMessage("I costi già registrati non vengono ricalcolati.").setView(container).setNegativeButton("Annulla", null).setPositiveButton("Salva", null).create()
