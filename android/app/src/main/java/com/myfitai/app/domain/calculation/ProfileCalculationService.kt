...
class ProfileCalculationService(
    ...
    data class BiaSnapshot(
        ...
        val bmr: MetricSnapshot,
        val fatMass: MetricSnapshot,
        val leanMass: MetricSnapshot,
        val bodyWaterKg: MetricSnapshot,
        val subcutaneousFat: MetricSnapshot,
        val boneMass: MetricSnapshot,
        val proteinPercent: MetricSnapshot,
        val proteinKg: MetricSnapshot,
        val bodyAge: MetricSnapshot,
        val bmi: MetricSnapshot,
    )
    ...
    suspend fun profileSnapshot(profileId: Long, today: LocalDate = LocalDate.now()): Snapshot? {
        ...
        val biaMetrics = BiaSnapshot(
            ...
            bmr = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.bmrKcal }),
            fatMass = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.fatMassKg }),
            leanMass = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.leanMassKg }),
            bodyWaterKg = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.bodyWaterKg }),
            subcutaneousFat = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.subcutaneousFatPercent }),
            boneMass = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.boneMassKg }),
            proteinPercent = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.proteinPercent }),
            proteinKg = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.proteinKg }),
            bodyAge = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.bodyAgeYears?.toFloat() }),
            bmi = metricSnapshot(biaHistory.map { it.measuredAtEpochMillis to it.bmi }),
        )
        val bodyMetrics = BodyMeasurementsSnapshot(
            ...
            hips = metricSnapshot(bodyHistory.map { it.measuredAtEpochMillis to it.hipsCm }),
