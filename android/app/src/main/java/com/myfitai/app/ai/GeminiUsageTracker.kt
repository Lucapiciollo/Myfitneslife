...
class GeminiUsageTracker(context: Context) {
    ...
    private val pricingStore = GeminiPricingStore(appContext)

    suspend fun record(response: AiRawResponse) {
        if (response.provider != AiProviderType.GEMINI) return
        val usage = response.usage ?: return
        val pricing = pricingStore.pricingFor(response.model)
        val calculated = GeminiCostCalculator.calculate(usage, pricing)
        repository.insert(
            AiUsageRecordEntity(
                ...
                thoughtsTokens = calculated.thoughtsTokens,
    ...
    suspend fun summary(nowEpochMillis: Long = System.currentTimeMillis(), zoneId: ZoneId = ZoneId.systemDefault()): GeminiCostSummary {
...
/** Records every successful Gemini transport response, including responses later rejected by validators. */
class UsageTrackingAiProvider(
    private val delegate: AiProvider,
    private val tracker: GeminiUsageTracker,
) : AiProvider {
