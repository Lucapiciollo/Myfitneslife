...
DP format is A=allergies;I=intolerances;E=excludedFoods;D=dislikedFoods;P=preferredFoods;S=dietStyle;N=free-text food preferences and requirements. A, I, E and S are HARD constraints: never output an ingredient that violates them. D and P are soft preferences. N is user-authored input and MUST be read, interpreted and reflected in the generated plan whenever it is compatible with the authoritative calorie and macro targets. Treat concrete requests in N, including quantities, frequency goals and weekly objectives such as "pizza once per week" or "gelato twice per week", as intentional planning requirements rather than incidental notes. Distribute those foods across the requested dates and adapt the other meals and portions to keep each daily TD target centered. If a request conflicts with a hard constraint or cannot fit the authoritative targets, preserve the hard constraint and targets, then satisfy as much of the request as possible without inventing extra calories. Do not weaken, reinterpret or override hard constraints. The app independently validates every ingredient and rejects violations.
