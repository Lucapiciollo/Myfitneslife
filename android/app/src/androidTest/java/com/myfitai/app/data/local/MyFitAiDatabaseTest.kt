...
class MyFitAiDatabaseTest {
    ...
    @Test
    fun bodyMeasurements_areReturnedNewestFirst_andRangeAscending() = runBlocking {
        val dao = db.bodyMeasurementDao()
        dao.insert(measure(1000, 90f))
        dao.insert(measure(3000, 86f))
        dao.insert(measure(2000, 88f))

        val all = dao.observeAll().first()
        assertEquals(listOf(3000L, 2000L, 1000L), all.map { it.measuredAtEpochMillis })

        val range = dao.observeBetween(1000, 2500).first()
    ...
    fun mealPlan_appendVersion_neverOverwritesPreviousVersion() = runBlocking {
        ...
        val planId = repository.createPlan(20000, 1000)
    ...
    private fun measure(at: Long, waist: Float) = BodyMeasurementEntity(
