...
- [x] Dialog Guida app uniformato agli aiuti: contenitore Material standard senza bordo verde esterno, card interne con raggio/bordo condivisi; guida allineata a target/consumo kcal e proteine. Contrasto secondario esplicito per `Registra sgarro` e `Fai capire lo sgarro all'IA`.
...
- [x] Diagnosi della schermata interna notifiche completata sul device: il percorso `Profilo -> Notifiche` e dieci cicli ripetuti apertura/ritorno non hanno prodotto blocchi, ANR o crash; la suite mirata notifiche/smoke è `7/7 PASS`. Restano da verificare separatamente consegna e tap delle notifiche Android reali.
- [x] Preferenza notifiche IA in background aggiunta e verificata: la modale notifiche controlla ora gli esiti dei job IA centralizzati, inclusa la generazione della dieta; disabilitarla silenzia solo le notifiche di successo/errore, senza fermare esecuzione o persistenza. Test preferenza/smoke connected `2/2 PASS` e verifica sul Samsung completata.
- [x] Prima tranche di uniformazione UI completata: token/primitive chiave-valore e stato, righe runtime, card dinamiche progressi/workout/history/nutrizione, binder IA, checklist BIA e card impatto misure sono state riallineate senza cambiare route o contratti. Build/unit test verdi e suite connected completa `69/69 PASS` su `SM-A546B - 16`; landscape, expanded-window e font-scale avanzato restano da verificare separatamente.
- [x] QA adattiva eseguita su device: landscape `2340x1080` e portrait con font scale `1.3` verificati sui percorsi Home/Profilo/Progresso/Rilevazioni con contenuto scrollabile, bottom navigation e righe chiave-valore senza clipping; impostazioni ripristinate a portrait/auto-rotation e font scale `1.1`. Aggiunta migration idempotente `12->13` per database correnti con colonne BIA/misure opzionali mancanti; migration test e suite connected completa `70/70 PASS`.
- [ ] Expanded/split-screen reale: il tentativo con override temporaneo `wm size 1600x1200` ha fatto perdere al device la registrazione dell'Activity (`Error type 3`), quindi il controllo app-level resta aperto e non viene marcato come verificato. Device ripristinato e smoke `1/1 PASS`.
- [x] Card e stati runtime residui uniformati: piano adattato, liste review e storico BIA usano ora primitive condivise chiave-valore/stato; build/unit test e suite connected completa restano verdi a `70/70 PASS`. Expanded/split-screen resta l'unico limite adattivo non certificato per instabilità del workaround `wm size` sul device.
- [x] Audit accessibilità/semantica completato sulle primitive condivise: righe chiave-valore e stato hanno descrizione unica leggibile, indicatori decorativi esclusi dall'albero, selettori giorno/intervallo con target minimo `48dp`; aggiunto test device dedicato. Suite connected completa `71/71 PASS`; expanded/split-screen resta l'unico limite non certificato.
- [x] Settings/Export runtime uniformati: scheduling automatico piano e analisi progressi usano spacing/typography condivisi, le azioni Export restano sulle row accessibili comuni e la sicurezza credenziali non è stata alterata. Targeted Settings/Security/Export `7/7 PASS`, suite connected completa `71/71 PASS`.
- [x] Audit globale verde nelle modali completato: overlay dialog con controlli radio/checkbox neutri, CTA positiva separata e layout lista/single-choice/multi-choice bianchi, start-aligned e con target `48dp`; coperti Lista spesa, Privacy, selezioni modello/frequenza/pasti/metrica e altri dialog comuni. Test mirati `6/6 PASS`, suite connected completa `74/74 PASS`.
- [x] Overlay Material Calendar/TimePicker allineati alle modali: superfici bianche; selezione giorno/anno su `surface_secondary` con testo `text_primary` ad alto contrasto; quadrante TimePicker bianco senza elevation overlay; controlli e lancetta neutri, CTA conferma `dialog_action_primary`. Test strumentale dedicato PASS e screenshot controllati su `SM-A546B - 16`.
- [x] Remediation chiarezza/gerarchia V1 verificata: `assembleDebug`, unit test e packaging AndroidTest PASS; `ProductClarityDeviceTest` 3/3 PASS e test guida standardizzata PASS rieseguiti su `SM-A546B - 16`. APK installato in-place; nessun Seeder/reset usato e nessun crash immediato osservato. Screenshot device di Alimentazione, Progresso e guida salvati negli artifact privati QA. Suite connected completa e confronto golden di tutte le schermate non sono stati eseguiti in questa tranche.
- [x] Form `Nuova misurazione` reso più leggibile e responsive: le misure bilaterali sono raggruppate in `Braccia`, `Cosce` e `Polpacci` con coppie `SX`/`DX`; su compact i gruppi sono impilati, da `w400dp` due gruppi condividono la riga. ID, binding e salvataggio invariati; build debug, unit test, `git diff --check` e `NewBodyMeasurementDeviceTest` `1/1 PASS` su `SM-A546B - 16`. Expanded/split-screen resta non certificato.
...
- [x] Analisi progressi IA: righe dettagli con semaforo circolare a colonna fissa e testo allineato sui wrapping; WorkManager viene riagganciato alla riapertura, la cadenza automatica è riprogrammata dopo il job riuscito, la persistenza sincrona del risultato precede `SUCCEEDED` e i test strumentali mirati layout/riapertura sono `2/2 PASS` sull’emulatore. L’esecuzione connessa Gradle ha ignorato il filtro device e coinvolto anche `SM-A546B`; non ripetere connected Gradle finché il filtro non è verificato, usare seriale adb esplicito.
- [x] Dettagli analisi progressi: colonna semaforo fissa con testo allineato anche sulle righe a capo; stato dei job WorkManager riagganciato alla riapertura, risultato persistito sincronicamente prima del completamento worker e cadenza automatica riprogrammata dopo successo. `assembleDebug`, unit test e packaging AndroidTest verdi; test strumentali mirati layout/riapertura `2/2 PASS` sull’emulatore. Non eseguire altri test connessi sul Samsung senza filtro seriale: il primo tentativo ha avviato i test su entrambi i device.
- [x] Test smoke strumentali `ActivitySmokeTest` adeguati alla UI semaforica con indicatore a contenitore fisso e colonna del testo indipendente; il test usa il campo Activity riflessivo perché il binder dinamico non ha un ID layout statico. Job-work temporaneo identificato da nome/tag congruenti, vincolo idle + delay e cancellazione nel `finally`, senza scrivere preferenze analisi. Suite mirata `2/2 PASS` su emulatore.
- [x] Micro-motion Views centralizzato in `UiMotion`: selezione giorni/segmenti, feedback checkbox, reveal stati/card e crossfade tab; animazioni brevi, interrotti/cancellabili, disattivate con animator scale a zero, risparmio energetico o feedback vocale. Righe lista spesa conservano le istanze tra aggiornamenti per evitare salti e ripetizioni; nessun cambio di business logic. Build debug, unit test e packaging AndroidTest PASS.
- [ ] QA visiva interattiva sul device dei micro-motion in Alimentazione, Lista spesa e Dettaglio pasto; installazione e avvio dell'app verificati, ma le singole transizioni non sono ancora certificate visivamente.
