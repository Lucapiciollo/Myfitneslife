...
**Stato reale (sessione QA su emulatore `emulator-5554`, build `assembleDebug`/`clean assembleDebug` verificate):**
- Portrait: TUTTE le 19 Activity reali dell'app sono state avviate e fotografate a schermo intero (`android:exported="true"` temporaneo su tutte tranne Splash, poi ripristinato — diff manifest verificato vuoto dopo il ripristino). Le 14 con un crop di riferimento dedicato (`99-screen-references/01..14`) sono state confrontate visivamente col mock in questa sessione: Splash/Onboarding, Home, Profilo, Misure corporee, BIA, Evoluzione fisica/Progressi, Piano alimentare, Dettaglio pasto, Lista spesa, Sgarro, Notifiche, Storico, Review settimanale — nessuna regressione rilevata rispetto alla verifica fatta schermata per schermata nelle sessioni precedenti.
- Le rimanenti Activity SENZA crop di riferimento nel mock (Allenamenti, Impostazioni, Piano adattato, Analisi IA, Export) sono state comunque ispezionate visivamente per coerenza con il design system (colori, spacing, componenti Material) e per bug oggettivi, NON per fedeltà pixel (nessun'immagine con cui confrontarle). Allenamenti e Impostazioni restano marcate **IMPLEMENTATE DA SPECIFICA — VERIFICA VISIVA PENDING** come da indicazione esplicita.
- **3 bug reali trovati e corretti in questa sessione:**
  1. Avatar profilo (`img_profile_avatar.png`): l'asset sorgente aveva il nome file ("avatar_defa...") impresso nei pixel in basso, visibile nel ritaglio circolare 64dp del profilo. Corretto ritagliando la fascia didascalia dall'immagine sorgente prima di copiarla in `res/drawable-nodpi`.
  2. `activity_adjusted_plan.xml`: testo del corpo card verde scuro (`bg_positive`) con colore `text_primary` (quasi nero su verde scuro), praticamente illeggibile. Corretto in `@color/white`.
  3. `activity_ai_analysis.xml`: stesso problema di contrasto in due punti — chip "30 giorni" selezionato (`bg_chip_selected` scuro + testo `#172229` scuro) e card viola "Consigli IA" (`bg_ai` scuro + testo `text_primary`). Entrambi corretti in `@color/white`.
- Landscape: verificato via rotazione reale dell'emulatore (`settings put system user_rotation 1`) su un campione rappresentativo di pattern di layout diversi — Home (header+chart+card), Piano alimentare (selettore settimana+righe), Misure corporee (figure affiancate in ConstraintLayout), BIA (TextInputLayout affiancati), Sgarro (ChipGroup a capo automatico). Nessun overflow, taglio o sovrapposizione rilevato in nessuno dei 5; contenuto scrollabile oltre la piega come atteso. NON verificato in landscape in modo esaustivo su tutte le 19 Activity (campione mirato sui pattern di layout più a rischio, non copertura 1:1).
- Dimensioni schermo differenti: verificato con `adb shell wm size`/`wm density` su una configurazione piccola (720x1520 @320dpi, ≈360x760dp — telefono compatto) e una grande (1200x1920 @240dpi, ≈800x960dp — tablet-ish), su un campione di 7 schermate diverse (Home, Profilo, Piano alimentare, Dettaglio pasto, Sgarro, Misure corporee, Lista spesa). Nessun overflow/testo tagliato in nessuna combinazione; su schermo grande resta spazio bianco inutilizzato in fondo ad alcune schermate (comportamento accettabile per un layout mobile-first, non un difetto). Anche qui: campione rappresentativo, non tutte le 19 Activity in tutte le dimensioni.
- Emulatore ripristinato a fine sessione: `wm size reset`, `wm density reset`, `accelerometer_rotation` riattivato, nessun flag `android:exported` temporaneo rimasto nel manifest (diff verificato vuoto).
- **Motivo per cui lo step 6 resta aperto (checkbox non spuntate):** la copertura landscape e multi-dimensione è su campione rappresentativo e non esaustiva su tutte le 19 Activity; le 5 schermate senza mock non sono "confrontate col mock" per definizione (non esiste un'immagine); non è stato eseguito un secondo giro di correzione dopo l'ultimo fix per escludere ulteriori bug minori non ancora trovati. Step 7 (Room) NON è stato iniziato.
...
- **IMPORTANTE**: verificato che `assets/MOCK_APPROVATO_MYFITAI_V2_COMPLETO.png` contiene solo 14 schermate numerate (non 16): non esiste un crop di riferimento pixel-perfect per "Allenamenti" né per "Impostazioni". Su indicazione esplicita dell'utente, il piano NON è stato ridotto da 16 a 14: le due schermate restano nel piano e sono state implementate seguendo le specifiche testuali già presenti (`03-screens/05-workouts.md`, `03-screens/11-export-settings.md`, `AGENTS.md`) e gli stessi token/componenti Material 3 delle altre 14, ma sono marcate come **IMPLEMENTATE DA SPECIFICA — VERIFICA VISIVA PENDING** (non "pixel-perfect verificate", perché non esiste un'immagine di riferimento con cui confrontarle).
- Allenamenti (`WorkoutsActivity`) ricostruita: nuovo `WeekDaySelectorView` (riuso) per la settimana Lun9-Dom15, nuovo componente riutilizzabile `WorkoutRowView` (foto reale per Pesi/Cardio da `workouts/workout_dumbbell.jpg` e `workout_running.jpg`, icona pausa `ic_pause` per i giorni di Riposo) con dati coerenti allo spec (data/tipo/durata/intensità percepita/orario). Verificato a runtime (Home → Profilo → Allenamenti): foto, icone e dati corretti.
- Impostazioni (`SettingsActivity`) ristilizzata a Material 3 SENZA alterare la logica di sicurezza reale preesistente (`SecureOpenAiKeyStore`, Android Keystore, `FLAG_SECURE`): `Switch`→`MaterialSwitch`, `EditText`→`TextInputLayout`/`TextInputEditText` con toggle password, pulsanti Salva/Rimuovi→`MaterialButton`, righe generali→`SettingRowView` riutilizzato con icone. Riga "Esporta dati" ora collegata realmente a `ExportActivity` (prima era raggiungibile solo con un long-press nascosto sul titolo, non discoverable).
- Nota tecnica: essendo `FLAG_SECURE` attivo, `adb screencap` produce un'immagine nera per questo schermo (comportamento di sicurezza corretto e atteso, non un bug) — verificato invece il contenuto/la struttura tramite dump `uiautomator` (tutte le stringhe attese presenti) e verificata via `dumpsys activity` la navigazione reale della riga Esporta dati verso `ExportActivity`.
