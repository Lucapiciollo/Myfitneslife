...
- `:app:connectedDebugAndroidTest` verde: 35 test eseguiti su `SM-A546B - 16`.
...
- La suite instrumented copre persistenza Room, riapertura di un database persistente temporaneo, piani legacy senza supplementi, migration 3 -> 4, export JSON/CSV ZIP/PDF inclusa apertura export dalla UI e creazione JSON, isolamento multiprofilo inclusi record annidati di piano, workflow AI deterministici con fake runtime provider-neutral, scheduler notifiche deterministico, lifecycle foto etichetta, UIAutomator E2E bottom-tab, stress dataset con tempi osservati e conservazione di supplementi/hydration note.
