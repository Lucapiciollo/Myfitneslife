...
- Never remove the demo app installation or its seeded demo data/profile; use in-place debug installs only and never clear app data during QA.
...
- Internal notification-screen diagnosis completed on `SM-A546B - 16`: the real `Profilo -> Notifiche` route opened `NotificationsActivity`, rendered the reminder clock/card/actions, and returned to `ProfileActivity` without `FATAL EXCEPTION` or ANR; ten repeated open/back cycles all completed successfully, and the targeted connected run passed `7/7` (`NotificationSchedulerTest`, `ReminderReceiverTest`, `ActivitySmokeTest`). No production change was required. The Android notification-shade permission/delivery path remains a separate open risk.
- Notification navigation correction implemented and device-verified: the `Profilo -> Notifiche` row now opens a `MaterialAlertDialog` configuration modal instead of navigating to the full-screen reminder `NotificationsActivity`; on `SM-A546B - 16`, the foreground remained `ProfileActivity`, the modal exposed meal reminders, weekly review, meal lead-time, `Salva` and `Annulla`, and its UI tree contained no `navHome`/`navMore` nodes. No `FATAL EXCEPTION` or ANR was observed. `:app:assembleDebug`, `:app:testDebugUnitTest`, APK install and the physical-device interaction check pass.
- Global dialog-surface correction completed and device-verified: Material 3 `MaterialAlertDialog` title and action panels were inheriting a lavender/rose `#ECE6F0` tone while custom content was white. The shared dialog theme now overrides the dialog container, title panel and button bar surfaces to white; the notification modal was rechecked on `SM-A546B - 16` with `#FFFFFF` samples in title/content/action regions, all controls present, `ProfileActivity` still foreground, and no crash/ANR. This applies to all `MaterialAlertDialogBuilder` modals, not only notifications.
- Background AI notification preference added and device-verified: `Profilo -> Notifiche` now includes `Aggiornamenti IA in background`, covering completion/failure notifications from the centralized `AiJobNotifier` for weekly plan generation and all other `AiJobType` WorkManager jobs. The preference silences only result notifications; AI execution and persistence continue. `NotificationPreferencesTest` and `ActivitySmokeTest` pass in the connected `2/2` run, and the Samsung modal exposes the new switch and description without crash/ANR.
- UI system audit group 1 completed: added shared runtime `KeyValueRowView` (left key/right value) and `StatusRowView` (left semantic indicator/center label/right state); aligned `MeasurementRowView`, `IngredientRowView`, `ShoppingItemRowView`, `SectionHeaderView` and `SettingRowView` spacing/typography; applied the status-row pattern to the Measurements diet-impact card; BIA condition checkboxes now use a shared Material checkbox token with 48dp minimum target and 14sp text. `:app:assembleDebug`, `:app:testDebugUnitTest`, `git diff --check`, and five connected tests (`ActivitySmokeTest`, `BiaDeviceVisualTest`, `MeasurementHistoryDeviceTest`, `FoodReviewScreensDeviceTest` plus their covered flows) pass on `SM-A546B - 16`. The wider Activity-by-Activity audit, landscape/font-scale verification and remaining runtime card migrations remain open.
- Full UI audit tranche completed on the current Views/XML architecture: shared key/value and status primitives now cover measurement impact, AI model/cost binders, nutrition supplements, and recurring workout/history/meal runtime rows; typography and min-height tokens were aligned; Physical Evolution dynamic cards use neutral zero-elevation surfaces; BIA checklist controls use the shared 48dp token. Full `connectedDebugAndroidTest` passes `69/69` on `SM-A546B - 16`; `:app:assembleDebug`, `:app:testDebugUnitTest` and `git diff --check` pass. Landscape, expanded-window and advanced font-scale visual QA remain explicitly unverified.
- Adaptive QA and schema repair completed: landscape rotation (`2340x1080`) rendered Home with scrollable content and distributed bottom navigation; portrait font scale `1.3` preserved Home metric/key-value rows, scrollability and bottom navigation without clipping; device settings were restored to portrait/auto-rotation and font scale `1.1`. During the run, a current-version device database with missing optional BIA/body columns was detected; added idempotent `MIGRATION_12_13`, migration test passes, and the full connected suite now passes `70/70` on `SM-A546B - 16`.
- Expanded-window follow-up attempted with temporary `wm size 1600x1200`: the Samsung launcher rendered the resized viewport, but Android deregistered the app Activity after the override (`Error type 3`), so app-level expanded-window verification is not considered certified. Physical display size/density, portrait rotation and font scale `1.1` were restored; APK reinstalled and `ActivitySmokeTest` passed `1/1` afterward.
- Runtime-card follow-up completed: adjusted-plan modified meals, weekly-review list items and BIA history dates now use shared status/key-value primitives; full connected suite remains `70/70 PASS` after the changes. Nutrition advice and review empty/loading/error flows were rechecked through their existing connected coverage. Expanded/split-screen remains the only unverified adaptive mode because the device resize workaround deregisters app Activities.
- Accessibility/semantics pass completed: `KeyValueRowView` and `StatusRowView` now expose a single readable `key: value`/`label: state` description while decorative indicators are excluded; time-range and weekday selectors use 48dp minimum targets and dynamic descriptions. Added `AccessibilitySemanticsDeviceTest`; targeted accessibility/smoke/BIA run passed `3/3`, and the full connected suite passes `71/71` on `SM-A546B - 16`.
- Settings/Export runtime pass completed: automatic nutrition-plan generation and automatic progress-analysis rows now use consistent 48dp settings spacing and shared settings typography; Export continues to use shared setting rows for import/export actions and protected credential behavior is unchanged. Settings/Security/Export targeted tests pass `7/7`, and the full connected suite remains `71/71` on `SM-A546B - 16`.
- Final shared-widget typography cleanup applied to `MealCardView`, `MeasurementActionCardView`, `MeasurementRowView` and `MetricCardView`, replacing remaining local font choices with shared TextAppearance tokens. `:app:assembleDebug` and `:app:testDebugUnitTest` pass after this change; the connected suite rerun is pending because `SM-A546B` disconnected and ADB showed no devices twice.
- Dialog-green audit completed: the shared dialog overlay now uses neutral `colorPrimary`/control colors for radio buttons, checkboxes and secondary actions, while only positive CTA text uses the dedicated dark-green action color. Added white/start-aligned 48dp dialog item layouts for normal, single-choice and multi-choice lists, covering Shopping List, Privacy, model/frequency/meal-count/BIA selections and other `MaterialAlertDialogBuilder` list dialogs. `DialogThemeDeviceTest` plus Settings/FoodReview targeted tests pass `6/6`; full connected suite passes `74/74` on `SM-A546B - 16`.
- Dialog control-color correction finalized: the shared dialog overlay now explicitly uses neutral control colors (`dialog_control_neutral`) and separate dark-green positive CTA (`dialog_action_primary`), eliminating inherited green radio/checkbox/list controls across Shopping List, Privacy, exit confirmation, BIA, model/frequency selections and all common dialogs. Full connected suite after the correction passes `74/74`; migration/export follow-up passes `15/15` when run with the generated schema assets.
- Demo dataset activated on device: debug `SEED_DEMO_12_MONTHS` completed and `SplashActivity` reopened Home with active profile `Demo 12 mesi` visible.
- Body-measurement acquisition screen compacted and unified: `activity_new_body_measurement.xml` now groups inputs into titled white cards `Dati rilevazione`, `Circonferenze` and `Arti`, removes the duplicated free-standing section headings, reduces repeated vertical gaps and preserves every input ID, scroll behavior and save/edit flow. Targeted device run (`NewBodyMeasurementDeviceTest`, `ActivitySmokeTest`, `MeasurementHistoryDeviceTest` and related checks) passes `4/4` on `SM-A546B - 16`.
- Bilateral body-measurement fields are now grouped semantically inside `Arti`: `Braccia`, `Cosce` and `Polpacci` each expose paired `SX`/`DX` inputs. Compact devices stack the groups; widths at least `w400dp` place two groups on the same row. All existing input IDs, edit/load/save binding and validation are unchanged. `:app:assembleDebug`, `:app:testDebugUnitTest`, `git diff --check` and `NewBodyMeasurementDeviceTest` (`1/1`) pass on `SM-A546B - 16` after an in-place debug install.
- Dialog/card alignment follow-up applied: shared dialog controls now use neutral colors globally; Shopping List empty/status text and Weekly Review status text are left-aligned, while only intentional controls/metric selectors remain centered. The subsequent connected run completed 74/75 tests; `StressPerformanceTest` was interrupted in teardown because the Samsung disconnected (`device not found`), not by an app assertion. Re-run the final connected suite when `SM-A546B` is online.
